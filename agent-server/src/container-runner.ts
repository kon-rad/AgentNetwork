...
function buildContainerArgs(
  ...
  // Inject agent ID so containers can authenticate with the credential proxy
  if (agentId) {
    args.push('-e', `AGENT_ID=${agentId}`);
    args.push(
      '-e',
...
export async function runContainerAgent(
  ...
  const containerArgs = buildContainerArgs(mounts, containerName, group.name);
